...
%% Get rid of first column of Theta2
Theta2NoBias = Theta2(:,2:end);
size(Theta2NoBias);
%%Theta2Mini = Theta2(:,2:end);
%%Theta2Mini = zeros(size(Theta2,1),size(Theta2,2)-1);
%%size(Theta2(:,2:end))


delta3 = zeros(m,num_labels);%% 5000 by 10
delta2 = zeros(m,size(Theta2NoBias,2));%% 5000 by 25

for t=1:m
  %%a1 = X(m,:);
  delta3(t,:) = a3(t,:) - y(t,:);
  %% IGNORE FIRST COLUMN OF THETA2 FOR THIS CALCULATION
  delta2(t,:) = ((Theta2NoBias' * delta3(t,:)') .* sigmoidGradient(z2(:,t)))';
  %%Get rid of delta2(0)
  %%delta2 = delta2(t,2:end);
  triangle1 = triangle1 + delta2(t,:)'*(X(t,:));
  triangle2 = triangle2 + delta3(t,:)'*(a2(t,:));
end
...
%% Now add a regularization term (remember do not regularize the bias)
